// ZoopUtility.cs (coroutines version)
// Replaced UniTask usage with Unity Coroutines and ZoopMod.Instance as runner
...
namespace ZoopMod.Zoop
{
  public class ZoopUtility
  {
      ...
      if (IsAllowed(InventoryManager.ConstructionCursor))
      {
        ...
        if (_cancellationToken == null)
        {
          ...
          if (Waypoints.Count > 0)
          {
            ...
            // Start the coroutine via your mod main MonoBehaviour
            if (ZoopMod.Instance != null)
            {
              ActionCoroutine = ZoopMod.Instance.StartCoroutine(ZoopCoroutine(inventoryManager));
            }
            else
            {
              Debug.LogError("[ZoopUtility] ZoopMod.Instance is null. Cannot start Zoop coroutine.");
    ...
    public static void CancelZoop()
    {
      ...
      if (_cancellationToken != null)
      {
        ...
        if (ActionCoroutine != null)
        {
          if (ZoopMod.Instance != null)
          {
            ZoopMod.Instance.StopCoroutine(ActionCoroutine);
          }
    ...
    private static IEnumerator ZoopCoroutine(InventoryManager inventoryManager)
    {
      ...
      while (isZooping && _cancellationToken != null && !_cancellationToken.IsCancellationRequested)
      {
        try
        {
          if (Waypoints.Count > 0)
        ...
        // 100 ms delay (previously UniTask.Delay(100, DelayType.Realtime))
        yield return new WaitForSecondsRealtime(0.1f);
      }

      // Ensure cleanup if loop exits naturally
      CancelZoop();
      yield break;
    }

    public static void StartBuildZoop(InventoryManager inventoryManager)
    {
      if (ZoopMod.Instance != null)
      {
        ZoopMod.Instance.StartCoroutine(BuildZoopCoroutine(inventoryManager));
      }
      else
      {
        Debug.LogError("[ZoopUtility] ZoopMod.Instance is null. Cannot start BuildZoop coroutine.");
      }
    }

    public static IEnumerator BuildZoopCoroutine(InventoryManager inventoryManager)
    {
      // Runs on main thread
      foreach (Structure item in structures)
      {
        ...
        // Emulate the previous behaviour with tiny delays between placements
        UsePrimaryComplete(inventoryManager, item);

        // small delay to mimic previous UniTask.Delay(10)
        yield return new WaitForSecondsRealtime(0.01f);
      }

      //Debug.Log("zoop canceled at BuildZoopCoroutine");
      CancelZoop();
      yield break;
    }
